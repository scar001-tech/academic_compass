...
import {
  ...
  Wifi, WifiOff, RefreshCw, School, User, LogOut,
} from "lucide-react";
...
const NAV = [
  ...
  { to: "/entry", label: "Offline Mark Entry", icon: PencilLine },
...
export default function AppShell() {
  const { state, activeCurriculum, setActiveCurriculum, setOnline, syncNow } = useSchool();
    ...
    <div className="min-h-screen bg-background text-foreground flex">
      ...
      <aside className="hidden lg:flex flex-col w-60 shrink-0 bg-sidebar text-sidebar-foreground border-r border-sidebar-border no-print">
        <div className="px-5 py-4 flex items-center gap-2 border-b border-sidebar-border">
          ...
          <div>
            <div className="text-sm font-semibold text-white">Scholaris</div>
          ...
          {NAV.map((n) => (
            ...
            >
              ...
              {n.to === "/conflicts" && conflicts > 0 && (
                <span className="text-[10px] rounded-full bg-destructive text-destructive-foreground px-1.5 py-0.5">{conflicts}</span>
              )}
              {n.to === "/entry" && pending > 0 && (
                <span className="text-[10px] rounded-full bg-warning text-warning-foreground px-1.5 py-0.5">{pending}</span>
              )}
            </NavLink>
          ))}
        </nav>
        ...
        <header className="sticky top-0 z-30 bg-card/95 backdrop-blur border-b no-print">
          <div className="px-4 lg:px-6 h-14 flex items-center gap-3">
            <div className="lg:hidden flex items-center gap-2">
              ...
              <div className="text-sm font-semibold">Scholaris</div>
            </div>

            <div className="ml-auto lg:ml-0 flex items-center gap-2">
              <Select value={activeCurriculum} onValueChange={(v) => setActiveCurriculum(v as any)}>
                  ...
                  {state.curricula.map((c) => (
                    <SelectItem key={c.id} value={c.id}>
                      <span className="inline-flex items-center gap-2">
                        <span className={cn("h-2 w-2 rounded-full", c.id === "cbc" ? "bg-[hsl(var(--curriculum-a))]" : "bg-[hsl(var(--curriculum-b))]")}/>
                        {c.name}
                      </span>
                    </SelectItem>
                  ))}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-2">
                    {state.online ? <Wifi className="h-4 w-4 text-success"/> : <WifiOff className="h-4 w-4 text-destructive"/>}
                    <span className="text-xs text-muted-foreground hidden sm:inline">
                      {state.online ? "Online" : "Offline"}
                    </span>
                    <Switch checked={state.online} onCheckedChange={setOnline} />
                  </div>
                </TooltipTrigger>
                <TooltipContent>Toggle connectivity (simulates offline mode)</TooltipContent>
              </Tooltip>

              {pending > 0 && (
                <Badge variant="outline" className="border-warning text-warning-foreground bg-warning-soft">
                  {pending} pending
                </Badge>
              )}
