    ...
    <div>
      ...
      <Tabs defaultValue="pending">
        ...
        <TabsContent value="pending" className="space-y-3 mt-3">
          {pending.length === 0 && <Card className="p-8 text-center text-muted-foreground"><CheckCircle2 className="h-6 w-6 mx-auto mb-2 text-success"/>No pending conflicts</Card>}
        ...
        <TabsContent value="resolved" className="space-y-3 mt-3">
          ...
          {resolved.map(c => (
            <Card key={c.id} className="p-3 text-sm flex items-center justify-between">
              ...
              <Badge variant="secondary">{c.resolution === "custom" ? c.customValue : (c as any)[`${c.resolution}Value`] || ""}</Badge>
            </Card>
          ))}
        </TabsContent>
      </Tabs>
...
function ConflictCard({ c, onResolve }: { c: any; onResolve: (id: string, r: any, custom?: string) => void }) {
  const { state } = useSchool();
  const [custom, setCustom] = useState("");
  const student = c.studentId ? state.students.find((s: any) => s.id === c.studentId) : null;
  const subject = c.subjectId ? state.subjects.find((s: any) => s.id === c.subjectId) : null;
  const exam = c.examId ? state.exams.find((e: any) => e.id === c.examId) : null;
...
function ChoiceBox({ label, value, meta, icon, onClick, highlight }: any) {
  return (
