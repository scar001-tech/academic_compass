...
import { Users, BookOpen, ClipboardList, FileSpreadsheet, AlertTriangle, TrendingUp, GitMerge, CheckCircle2 } from "lucide-react";
...
export default function Dashboard() {
  ...
  const pendingMarks = state.entries.filter(e => {
    const sh = state.sheets.find(x => x.id === e.sheetId);
    return sh?.curriculumId === activeCurriculum && (e.score == null || e.pending);
  }).length;
  ...
  // Subject averages for latest closed/open exam
  ...
  // Trend across exams (overall mean)
        ...
        <Card className="p-4 lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="text-xs text-muted-foreground">{latestExam ? `${latestExam.name} · Term ${latestExam.term}` : "No exam data"}</div>
            </div>
        ...
        <Card className="p-4">
          ...
          <div className="overflow-x-auto">
            <table className="data-table">
              <thead><tr><th>Subject</th><th>Class</th><th>Exam</th><th>Status</th></tr></thead>
              <tbody>
          ...
          <ul className="space-y-2 text-sm">
            {conflicts > 0 && (
              <li className="flex items-center justify-between rounded-md border p-3">
                <span className="flex items-center gap-2"><GitMerge className="h-4 w-4 text-destructive"/> {conflicts} sync conflict{conflicts>1?"s":""} awaiting review</span>
                <Button asChild size="sm" variant="outline"><Link to="/conflicts">Resolve</Link></Button>
              </li>
            )}
            {pendingMarks > 0 && (
              <li className="flex items-center justify-between rounded-md border p-3">
                <span className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-warning"/> {pendingMarks} pending mark entries</span>
                <Button asChild size="sm" variant="outline"><Link to="/entry">Open</Link></Button>
              </li>
            )}
            {weakAreas > 0 && (
              <li className="flex items-center justify-between rounded-md border p-3">
                <span className="flex items-center gap-2"><TrendingUp className="h-4 w-4 text-destructive"/> {weakAreas} subject{weakAreas>1?"s":""} below 50% average</span>
                <Button asChild size="sm" variant="outline"><Link to="/transcripts">Review</Link></Button>
              </li>
            )}
            {conflicts + pendingMarks + weakAreas === 0 && (
              <li className="text-sm text-muted-foreground">All clear. Nice work.</li>
            )}
          </ul>
        </Card>
      </div>
