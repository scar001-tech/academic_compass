...
export default function MarkSheets() {
  ...
  const setStatus = (id: string, status: any) => update(s => {
    const sh = s.sheets.find(x => x.id === id); if (sh) { sh.status = status; sh.updatedAt = Date.now(); }
  });
            ...
            {sheets.map(sh => {
              ...
              return (
                <tr key={sh.id}>
                  ...
                  <td>
                    <select className="inline-edit text-xs" value={sh.status}
