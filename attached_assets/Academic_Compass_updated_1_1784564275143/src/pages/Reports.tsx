...
export default function Reports() {
  ...
  const printAll = () => {
    // Bulk print prompt: keep it simple — advise user
    window.print();
  };
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-1 no-print">
            ...
            <Button size="sm" onClick={printAll}><Printer className="h-4 w-4 mr-1"/>Print</Button>
          </div>
        }
      />
      ...
      {student && exam && stats && (
        <div className="a4-sheet print-page">
          ...
          <header className="flex items-center gap-4 border-b pb-4">
            <div className="h-16 w-16 rounded-lg bg-primary text-primary-foreground grid place-items-center">
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-3 py-3 text-sm border-b">
            <Field label="Student name" value={student.name} disabled={!canManageStudents} onChange={(v) => update(s => { const x = s.students.find(x => x.id === student.id); if (x) x.name = v; })}/>
            <Field label="Admission No." value={student.admissionNo} disabled={!canManageStudents} onChange={(v) => update(s => { const x = s.students.find(x => x.id === student.id); if (x) x.admissionNo = v; })}/>
            <div>
          ...
          <section className="py-3 border-b">
            ...
            <Textarea className="text-sm min-h-[60px]" defaultValue={classRemark?.remark || state.settings.classTeacherRemarkTemplate}
              onBlur={(e) => update(s => {
                ...
                if (idx >= 0) s.classRemarks[idx].remark = e.target.value;
                else s.classRemarks.push({ studentId: student.id, examId: exam.id, remark: e.target.value, teacherName: classTeacher?.name || "", updatedAt: Date.now() });
              })}/>
            <div className="text-xs text-muted-foreground mt-1">Signed: {classTeacher?.name || "—"} · Date: {new Date().toLocaleDateString()}</div>
          </section>

          {/* 5. Principal remarks — MUST be last */}
          <section className="py-3">
            ...
            <Textarea className="text-sm min-h-[60px]" defaultValue={principalRemark?.remark || state.settings.principalRemarkTemplate}
              ...
              onBlur={(e) => update(s => {
                ...
                if (idx >= 0) s.principalRemarks[idx].remark = e.target.value;
                else s.principalRemarks.push({ studentId: student.id, examId: exam.id, remark: e.target.value, principalName: "Dr. Joseph Mwangi", updatedAt: Date.now() });
              })}/>
            <div className="text-xs text-muted-foreground mt-1">Signed: {state.teachers.find(t => t.role === "principal")?.name || "Principal"} · Date: {new Date().toLocaleDateString()}</div>
          </section>
        </div>
      )}
    </div>
